...
export default class DbService<DbEntityType> {
    ...
    private getStorageAccountConnectionString(): string {
        const connectionString = this.storageAccountConnectionString || process.env.STORAGE_ACCOUNT_CONNECTION_STRING;
        if (!connectionString) {
            throw new Error("STORAGE_ACCOUNT_CONNECTION_STRING is not set");
        }
        this.storageAccountConnectionString = connectionString;
        return connectionString;
    }

    async getEntityByRowKey(tableName: string, rowKey: string): Promise<DbEntity> {
        if (!this.okToCacheLocally) {
            const tableClient = TableClient.fromConnectionString(this.getStorageAccountConnectionString(), tableName);
